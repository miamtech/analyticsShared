...
import mealz.ai.Analytics.Companion.PLAUSIBLE_URL
...
public class AnalyticsImp : Analytics {
    ...
    private suspend fun HttpClient.postEvent(event: Analytics.PlausibleEvent) {
...
//        MealzDI.analyticsNotifier.push(event)
    ...
    override fun sendEvent(eventType: String, path: String) {
        val props = Analytics.PlausibleProps()
...
//        val propsWithVersionAndDevice = props.copy(version = SdkVersion.VERSION, device = getOperatingSystem().name)
        ...
        val event = Analytics.PlausibleEvent(
            eventType,
            url,
            domain,
    ...
    fun sendRecipeDisplay(recipeId: String, path: String) {
